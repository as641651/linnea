...
int main(int argc, char ** argv)
{
    ...
    std::cout << "Test runner for algorithm tmp" << std::endl;
    linalg_tests::basic_benchmarker benchmark;
    benchmark.set_cache_size(30 * 1024 * 1024);
    std::ofstream file("cpp_results_tmp_timings.txt");
    {
    ...
    std::array<std::string, 2> labels_arma_n{ {"naive_armadillo", "24"} };
    benchmark.run(file, labels_arma_n, 20, [&]() {
            call(naive_armadillo{}, matrices, Indices{});
            });

    std::array<std::string, 2> labels_arma_r{ {"recommended_armadillo", "24"} };
    ...
    std::array<std::string, 2> labels_eigen_n{ {"naive_eigen", "24"} };
    benchmark.run(file, labels_eigen_n, 20, [&]() {
            call(naive_eigen{}, matrices, Indices{});
            });

    std::array<std::string, 2> labels_eigen_r{ {"recommended_eigen", "24"} };
