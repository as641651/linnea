...
int main(int argc, char ** argv)
{
    ...
    std::array<std::string, 2> labels_arma_n{ {"naive_armadillo", "1"} };
    benchmark.run(file, labels_arma_n, 20, [&]() {
            call(naive_armadillo{}, matrices, Indices{});
            });

    std::array<std::string, 2> labels_arma_r{ {"recommended_armadillo", "1"} };
    ...
    std::array<std::string, 2> labels_eigen_n{ {"naive_eigen", "1"} };
    benchmark.run(file, labels_eigen_n, 20, [&]() {
            call(naive_eigen{}, matrices, Indices{});
            });

    std::array<std::string, 2> labels_eigen_r{ {"recommended_eigen", "1"} };
