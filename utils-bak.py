...
class Algorithm():
    ...
    def code(self):
        ...
        ## ADDED BY ARAVIND
        code_list.append("\n".join(["{}sizeArg {}:{}".format(config.comment, self.memory.lookup[operand.name].name, self.memory.lookup[operand.name].size) for operand in input_operands]))
        code_list.append("\n")
        #####################
    ...
    def _matched_kernel_to_code(self, matched_kernel, line_number):
        ...
        if mem_ops_before:
            ## ADDED BY ARAVIND
            for mem_op in mem_ops_before:
               try:
                  lines_list.append("\n{}sizeArg {}:{}".format(config.comment, mem_op.mem_loc.name, mem_op.mem_loc.size))
               except AttributeError:
                  if isinstance(mem_op, memory_module.ConvertStorageFormat):
        ...
        ### ADDED BY ARAVIND
        lines_list.append("{}cost_K {:.3g} oprs_K ".format(config.comment, matched_kernel.cost))
